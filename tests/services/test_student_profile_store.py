...
def test_record_diagnostic_result_updates_profile_history(state_file: Path) -> None:
    ...
    profile = store.record_diagnostic_result(
        ...
        current_topic_id="g2_t01",
        current_grade=2,
        diagnosis_result_id="diag_2_5_1",
        diagnosis_confidence=0.8,
        weak_topic={"topic_id": "g2_t01", "topic": "Сложение двузначных чисел"},
        detected_gaps=["g2_addition_core"],
    )

    assert profile["activity_counters"]["diagnostic_sessions"] == 1
    assert profile["active_scope"]["current_skill_id"] == "g2_addition_core"
    assert profile["diagnostic_history"]
    assert profile["diagnostic_history"][0]["diagnosis_result_id"] == "diag_2_5_1"
