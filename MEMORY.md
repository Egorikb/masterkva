...
Экономный порядок исполнения и отдельные ТЗ моделей: [план завершения](docs/EXECUTION_PLAN_2026-09-22/README.md). U1a принята; следующий обязательный этап — **H0: наблюдение владельца и педагога** по локальному сценарию. C0/C1 остаются независимыми READY-карточками. Astra — только спорный узел или итоговая независимая проверка.
...
- lesson_assessment.py — чистая оценка по частям/рубрикам. Узкий B1 принят: только `g1-t12-l01` v2.0 в `/api/v1/plugins/panda/chat`; остальные 77 уроков закрыты, mastery не повышается. B2 принят: [действия, request/session/part IDs и повторы](docs/api/panda-actions.md) определяет сервер. U1a подключила этот урок к локальному текстовому UI; [сценарий H0 и приёмка](docs/audit/2026-09-23-u1a-acceptance.md).
