    ...
    try:
        while True:
            ...
            try:
                ...
                # Parse visual from response
                visual_data = None
                import re, json
                visual_match = re.search(r'\{"visual\":\s*\{', full_response)
                if visual_match:
                    try:
                        # Extract JSON block
                        start = visual_match.start()
                        json_str = full_response[start:start+500]
                        # Find the JSON block
                        import json
                        # Simple parse - just extract visual field
                        if '"bar_model"' in json_str:
                            parts = re.findall(r'\d+', json_str)
                            visual_data = {"type": "bar_model", "total": int(parts[0]) if parts else 8, "parts": [int(p) for p in parts[:2]] if parts else [5, 3]}
                        elif '"ten_frame"' in json_str:
                            parts = re.findall(r'\d+', json_str)
                            visual_data = {"type": "ten_frame", "filled": int(parts[0]) if parts else 5, "total": 10}
                    except Exception as e:
                        logger.debug(f"Visual parse error: {e}")
