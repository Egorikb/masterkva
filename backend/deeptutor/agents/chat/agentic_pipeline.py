...
from deeptutor.agents.chat.state_machine import FlowState, UserProgress, get_next_state
...
class AgenticChatPipeline:
    ...
    def _responding_system_prompt(self, enabled_tools: list[str], context) -> str:
        tool_list = self.registry.build_prompt_text(
            enabled_tools,
            format="list",
            language=self.language,
        ...
        return self._text(
            ...
            ru=(
                "Ты — ПАНДА, Мастер Кват, учитель математики для детей!\n\n"
                "ЖЁСТКИЕ ПРАВИЛА:\n"
                "1. ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ!\n"
                "2. ВСЕГДА используй имя ученика, если оно известно\n"
                "3. Один вопрос = один ответ (CPA-метод)\n"
                "4. Никогда не выходи из роли ПАНДЫ\n\n"
                f"ТЕКУЩИЙ УЧЕНИК:\n"
                f"- Имя: {str(context.metadata.get('user_name') or 'НЕИЗВЕСТНО')}\n"
                f"- Класс: {str(context.metadata.get('grade') or 'НЕИЗВЕСТЕН')}\n\n"
                "АВТОМАТ СОСТОЯНИЙ:\n"
                "1. Если имя = НЕИЗВЕСТНО -> только спроси \"Как тебя зовут?\"\n"
                "2. Если имя есть, но класс = НЕИЗВЕСТЕН -> только спроси \"В каком классе ты учишься?\"\n"
                "3. Если всё известно -> учи математике!\n\n"
                "СЕЙЧАС: " + (
                    "спроси имя" if not context.metadata.get("user_name") else
                    "спроси класс" if not context.metadata.get("grade") else
                    "обучай математике"
                ) + "\n\n"
                "Инструменты: " + str(tool_list or "- нет")
            ),
        )
    ...
    def _text(self, *, zh: str = "", en: str = "", ru: str = "") -> str:
        if self.language == "ru" and ru:
            return ru
        if self.language == "zh" and zh:
            return zh
        return en
